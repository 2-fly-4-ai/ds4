...
tag = f'mtp{int(a.mtp)}-think{int(a.thinking)}'
...
for rep in range(a.rounds):
    for mode in (('base','candidate') if rep%2 == 0 else ('candidate','base')):
        work = root if mode == 'base' else candidate
        ...
        cmd = [str(work/'ds4-server'), '-m',str(root/'gguf/Qwen3.8-Flash-Next-DS4-Ivan-Q4-Compat.gguf'),
               '--host','127.0.0.1','--port',str(port),'--ctx','8192','--trace',str(prefix)+'.trace']
        if a.mtp: cmd += ['--mtp']
        with open(str(prefix)+'.log','w') as log:
            ...
            try:
                ...
                for stream in (False, True):
                    system = 'You are a coding assistant. Use read_file to inspect the requested file, then report its project version in one short sentence. Do not invent file contents.\n'
                    system += '\n'.join(f'Reference item {i}: preserve public interfaces and test changes before release.' for i in range(100))
                    msgs = [{'role':'system','content':system}, {'role':'user','content':f'Case {int(stream)}: read package.json and tell me the project version.'}]
                    ...
                    for turn in (0,1):
                        ...
                        if turn == 0:
                            ...
                            assert json.loads(calls[0]['function']['arguments'])['path']=='package.json', msg
                            ...
                            msgs.append({'role':'tool','tool_call_id':calls[0]['id'],'content':'{"name":"fixture-project","version":"2.7.4"}'})
                        else:
                            assert '2.7.4' in (msg.get('content') or '') and not msg.get('tool_calls'), msg
            finally:
                proc.terminate()
