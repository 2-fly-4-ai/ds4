...
static void print_model_runtime(FILE *fp, const help_colors *c,
    ...
    opt(fp, c, "--ple FILE", "Qwen external PLE table; kept CPU-mapped outside Metal residency.");
    opt(fp, c, "--engram FILE", "DeepSeek V4.1 Engram tables; kept CPU-mapped outside Metal residency.");
    ...
    if (full) {
        ...
        if (tool == DS4_HELP_DS4 || tool == DS4_HELP_AGENT || tool == DS4_HELP_SERVER) {
            ...
            opt(fp, c, "--dspark", "Enable DSpark using the support GGUF passed with --mtp-model. Immediate no-draft skip is off; scheduler window=8 min-avg=2.0. One-token proposals use ordinary decode.");
...
static void print_bench_specific(FILE *fp, const help_colors *c) {
    ...
    opt(fp, c, "--dump-decode-logits-dir DIR", "Write raw F32 logits after every measured decode step.");
    fputc('\n', fp);
}
