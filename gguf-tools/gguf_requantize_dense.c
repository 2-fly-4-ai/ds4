...
/* GLM-5.3 has two attention families.  Its recurrent KDA q/k matrices are
 * already Q4_K in the released Q2 artifact; the remaining KDA projections
 * and the sparse DSA projections are Q8_0.  Keep the BF16 indexer out of this
 * family because changing it can alter long-context token selection. */
static int tensor_block_index(const char *name) {
    int block = -1;
    return name && sscanf(name, "blk.%d.", &block) == 1 ? block : -1;
}

static bool is_glm53_target_block_tensor(const char *name, int mtp_block) {
    if (strncmp(name, "blk.", 4) != 0 || strstr(name, "indexer")) return false;
    /* The highest GLM-5.3 block is the embedded next-token predictor.  Keep it
     * unchanged so this experiment measures target-model attention/head
     * quantization without also perturbing MTP acceptance. */
    return tensor_block_index(name) != mtp_block;
}

static bool has_tensor_suffix(const char *name,
...
static bool is_glm53_dsa_projection(const char *name, int mtp_block) {
    ...
    return is_glm53_target_block_tensor(name, mtp_block) &&
           has_tensor_suffix(name, suffixes, sizeof(suffixes) / sizeof(suffixes[0]));
}

static bool is_glm53_kda_projection(const char *name, int mtp_block) {
    static const char *const suffixes[] = {
        ".kda_v.weight",
        ".kda_f_a.weight",
        ".kda_beta.weight",
...
static bool is_glm53_attention_projection(const char *name, int mtp_block) {
    return is_glm53_dsa_projection(name, mtp_block) ||
           is_glm53_kda_projection(name, mtp_block);
}
...
static void usage(const char *argv0) {
    ...
    printf("  --glm53-attention T  re-encode GLM-5.3 KDA/DSA attention projections\n");
    printf("  --glm53-kda T        re-encode only GLM-5.3 recurrent KDA projections\n");
    printf("  --glm53-dsa T        re-encode only GLM-5.3 sparse full-attention projections\n");
...
int main(int argc, char **argv) {
    ...
    ds4q_type glm53_attn_type = DS4Q_TYPE_COUNT;
    ds4q_type glm53_kda_type = DS4Q_TYPE_COUNT;
    ds4q_type glm53_dsa_type = DS4Q_TYPE_COUNT;
    ...
    for (int i = 1; i < argc; i++) {
        ...
        else if (!strcmp(arg, "--glm53-attention") && i + 1 < argc) glm53_attn_type = parse_type(argv[++i]);
    ...
    int glm53_mtp_block = -1;
    if (glm53_attn_type != DS4Q_TYPE_COUNT ||
        glm53_kda_type != DS4Q_TYPE_COUNT ||
        glm53_dsa_type != DS4Q_TYPE_COUNT) {
        for (uint64_t i = 0; i < g.n_tensors; i++) {
            const int block = tensor_block_index(g.tensors[i].name);
            if (block > glm53_mtp_block) glm53_mtp_block = block;
        }
    }
    ...
    for (uint64_t i = 0; i < g.n_tensors; i++) {
        ...
        if (target == t->type &&
            is_glm53_attention_projection(t->name, glm53_mtp_block) &&
            glm53_attn_type != DS4Q_TYPE_COUNT) {
            target = glm53_attn_type;
        }
        if (target == t->type &&
            is_glm53_kda_projection(t->name, glm53_mtp_block) &&
            glm53_kda_type != DS4Q_TYPE_COUNT) {
            target = glm53_kda_type;
        }
        if (target == t->type &&
            is_glm53_dsa_projection(t->name, glm53_mtp_block) &&
            glm53_dsa_type != DS4Q_TYPE_COUNT) {
            target = glm53_dsa_type;
        }
