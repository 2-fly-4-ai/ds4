...
static slot_reuse slot_probe_reuse_locked(server *s, server_slot *slot,
    ...
    /* Token or visible-text equality is insufficient for multimodal state:
     * generic image placeholders can represent different embeddings. Keep
     * image identity in the one reuse verdict shared by routing and execution. */
    ...
    /* A forward continuation is already the cheapest reuse tier and must not
     * be mistaken for a rewind by the generalized Flash reuse policy. */
    ...
    if (s->engine && ds4_engine_is_glm_dsa(s->engine)) {
         ...
         * DeepSeek V4.1 has the same restriction for its recurrent attention
         * compressor and indexer state; its safe hot-frontier snapshot is not
         * implemented yet, so never advertise a token-only rewind for it. */
...
/* Streaming the first generated token immediately preserves interactive
 * latency.  After that, coalesce a few tokens so JSON framing and socket writes
 * do not repeatedly stall the chained Metal callback.  The time cap matters at
 * long contexts where a nominal two-token batch could otherwise sit pending
 * for noticeably longer than it does at the model's normal decode rate. */
static int server_stream_token_batch(void) {
    int batch = 2;
    const char *env = getenv("DS4_STREAM_TOKEN_BATCH");
    if (env && env[0]) {
        char *end = NULL;
        long v = strtol(env, &end, 10);
        if (end != env && *end == '\0' && v >= 1 && v <= 32) batch = (int)v;
    }
    return batch;
}

static double server_stream_max_delay(void) {
    long ms = 75;
    const char *env = getenv("DS4_STREAM_MAX_DELAY_MS");
    if (env && env[0]) {
        char *end = NULL;
        long v = strtol(env, &end, 10);
        if (end != env && *end == '\0' && v >= 0 && v <= 1000) ms = v;
    }
    return (double)ms / 1000.0;
}

static bool server_stream_should_flush(const request *r,
                                       int completion,
                                       int last_flush_completion,
                                       int token_batch,
                                       double last_flush_t,
                                       double now,
                                       double max_delay) {
    if (!r || !r->stream || completion <= last_flush_completion) return false;
    if (completion == 1) return true;
    if (completion - last_flush_completion >= token_batch) return true;
    return max_delay > 0.0 && now - last_flush_t >= max_delay;
}

/* Keep the server chain path deliberately narrow.  The chained Metal graph is
 * greedy-only and its callback runs on the generation thread, so protocol
 * features which can intervene between individual tokens stay on the classic
 * loop below.  Streaming is safe because the callback feeds the exact same SSE
 * state machines as the classic loop.  Textual stop sequences and tools remain
 * excluded because they can trim or reinterpret generated bytes.  Thinking is
 * also safe: unlike ds4-eval, the server does not inject a forced close token,
 * and the callback feeds thinking_state for every approved argmax token. */
static bool server_greedy_chain_request_shape_eligible(
        const server *s, const request *r, int max_tokens, int room) {
    return s && r &&
           !s->batched_mode &&
           !r->has_tools &&
           r->stops.len == 0 &&
           (!ds4_think_mode_enabled(r->think_mode) || r->temperature_set) &&
           r->temperature <= 0.0f &&
           max_tokens >= 2 &&
           /* Run one extra eval, then reject its predicted token.  That leaves
            * every returned token evaluated into KV even at the output limit. */
           room > max_tokens;
}
...
typedef struct {
    ...
    int *completion;
    int *next_decode_log;
    int stream_token_batch;
    int *last_stream_flush_completion;
    bool responses_protocol;
    ...
    double decode_t0;
    double stream_max_delay;
    double *last_stream_flush_t;
...
static bool server_greedy_chain_on_token(void *ud, int token) {
    server_greedy_chain_ctx *c = ud;
    if (g_stop_requested) {
        c->shutdown = true;
        return false;
    }
    if (job_cancelled(c->request_job)) {
        c->cancelled = true;
        return false;
    }
    /* The extra predicted token exists only to evaluate the final returned
     * token into KV.  Do not expose it or add it to the session checkpoint. */
    if (*c->completion >= c->limit) {
        c->hit_limit = true;
        return false;
    }
    ...
    size_t piece_len = 0;
    char *piece = ds4_token_text(c->srv->engine, token, &piece_len);
    trace_piece(c->srv, c->trace_id, piece, piece_len);
    buf_append(c->text, piece, piece_len);
    thinking_state_feed(c->thinking, piece, piece_len);
    free(piece);
    (*c->completion)++;

    const request *r = &c->request_job->req;
    double stream_now = r->stream ? now_sec() : 0.0;
    if (server_stream_should_flush(r, *c->completion,
                                   *c->last_stream_flush_completion,
                                   c->stream_token_batch,
                                   *c->last_stream_flush_t,
                                   stream_now,
                                   c->stream_max_delay)) {
        size_t stream_len = utf8_stream_safe_len(c->text->ptr,
                                                  *c->plain_stream_pos,
                                                  c->text->len, false);
        bool stream_ok = true;
        if (!c->structured_stream && stream_len > *c->plain_stream_pos) {
            char *delta = xstrndup(c->text->ptr + *c->plain_stream_pos,
                                  stream_len - *c->plain_stream_pos);
            stream_ok = sse_chunk(c->request_job->fd, r, c->id, delta, NULL);
            free(delta);
            if (stream_ok) *c->plain_stream_pos = stream_len;
        }
        if (stream_ok && r->api == API_ANTHROPIC) {
            stream_ok = anthropic_sse_stream_update(
                c->request_job->fd, c->srv, r, c->id,
                c->anthropic_live, c->text->ptr, stream_len, false);
        }
        if (stream_ok && c->openai_live_chat) {
            stream_ok = openai_sse_stream_update(
                c->request_job->fd, c->srv, r, c->id,
                c->openai_live, c->text->ptr, stream_len, false);
        }
        if (stream_ok && c->responses_live_chat) {
            stream_ok = responses_sse_stream_update(
                c->request_job->fd, r, c->responses_live,
                c->text->ptr, stream_len, false);
        }
        if (!stream_ok) {
            job_mark_cancelled(c->request_job);
            c->cancelled = true;
            return false;
        }
        *c->last_stream_flush_completion = *c->completion;
        *c->last_stream_flush_t = stream_now;
    }

    if (*c->completion >= *c->next_decode_log) {
        log_decode_progress(c->kind, c->prompt_tokens, *c->completion,
                            c->responses_protocol,
                            false, c->thinking->inside, false, false,
                            c->decode_t0,
                            c->last_decode_log_t,
                            c->last_decode_log_completion);
        *c->next_decode_log += 50;
    }
    return true;
}
...
static int server_prefill_quantum_for(const server *s,
     ...
     * cap (4096 for Flash, 8192 for PRO) instead of halving it. Every partial
     * chunk pays the same fixed MoE expert scan as a full one. */
...
static void generate_job_inner(server *s, server_slot *slot, job *j) {
    ...
    const int responses_src_visible = reuse.kind == REUSE_RESPONSES_VISIBLE;
    const int responses_src_tool_output =
    ...
    case REUSE_MEMORY_REWIND:
        if (!ds4_engine_is_glm_dsa(s->engine) &&
            s->kv.enabled && old_pos >= s->kv.opt.min_tokens) {
        ...
        slot->continued_last_store_tokens = reuse.reuse_tokens;
        server_log(DS4_LOG_KVCACHE,
                   "ds4-server: live kv cache rewind-hit live=%d prompt=%d common=%d reused=%d",
                   old_pos, j->req.prompt.len, common, reuse.reuse_tokens);
        break;
...
decode_again:
    ...
    int last_stream_flush_completion = 0;
    if (max_tokens < 0) max_tokens = 0;
    if (max_tokens > room) max_tokens = room;
    trace_event(s, trace_id, "prefill done; decode_max=%d ctx_room=%d", max_tokens, room);
    const double decode_t0 = now_sec();
    const int stream_token_batch = server_stream_token_batch();
    const double stream_max_delay = server_stream_max_delay();
    double last_stream_flush_t = decode_t0;
...
hybrid_chain_again:
        ...
        ds4_session_chain_greedy_supported(slot->session))
    {
        server_greedy_chain_ctx chain = {
            ...
            .next_decode_log = &next_decode_log,
            .stream_token_batch = stream_token_batch,
            .last_stream_flush_completion = &last_stream_flush_completion,
            .responses_protocol = responses_protocol,
        ...
        server_log(DS4_LOG_GENERATION,
                   "ds4-server: %s ctx=%s greedy Metal chain start max=%d",
                   j->req.kind == REQ_CHAT ? "chat" : "completion",
                   ctx_span, max_tokens);
        pthread_mutex_lock(&s->inference_mu);
        int chain_tokens = ds4_session_eval_chain_greedy(
            slot->session, max_tokens + 1,
        ...
        } else if (!chain.cancelled &&
                   !(chain.hit_limit && completion == max_tokens)) {
            finish = "error";
            snprintf(err, sizeof(err),
                     "greedy Metal chain ended unexpectedly (%d tokens, completed=%d)",
                     chain_tokens, chain_completed ? 1 : 0);
        }
        trace_event(s, trace_id,
                    "greedy Metal chain done approved=%d generated=%d stop=%d limit=%d completed=%d",
                    chain_tokens, completion, chain.hit_stop ? 1 : 0,
                    chain.hit_limit ? 1 : 0, chain_completed ? 1 : 0);
        if (!chain.cancelled && strcmp(finish, "error") != 0) {
            kv_cache_maybe_store_continued(s, slot);
        }
    }
    while (!greedy_chain_handled &&
           !g_stop_requested && !job_cancelled(j) && completion < max_tokens &&
        ...
        for (int ti = 0; ti < ntok && completion < max_tokens; ti++) {
            ...
            const double stream_now = j->req.stream ? now_sec() : 0.0;
            const bool flush_stream = j->req.stream &&
                (hit_stop || server_stream_should_flush(
                    &j->req, completion, last_stream_flush_completion,
                    stream_token_batch, last_stream_flush_t,
                    stream_now, stream_max_delay));

            if (flush_stream && !structured_stream &&
            ...
            if (flush_stream && j->req.api == API_ANTHROPIC &&
            ...
            if (flush_stream && openai_live_chat &&
            ...
            if (flush_stream && responses_live_chat &&
            ...
            if (flush_stream) {
                last_stream_flush_completion = completion;
                last_stream_flush_t = stream_now;
            }
            free(piece);
...
static void test_greedy_chain_request_shape_guards_protocol_features(void) {
    server s = {0};
    request r = {0};
    r.temperature = 0.0f;
    r.think_mode = DS4_THINK_NONE;

    TEST_ASSERT(server_greedy_chain_request_shape_eligible(&s, &r, 64, 128));
...
static void test_server_stream_batch_preserves_first_token_and_time_cap(void) {
    request r = {0};
    TEST_ASSERT(!server_stream_should_flush(&r, 1, 0, 4,
                                            10.0, 10.0, 0.075));
    r.stream = true;
    TEST_ASSERT(server_stream_should_flush(&r, 1, 0, 4,
                                           10.0, 10.0, 0.075));
    TEST_ASSERT(!server_stream_should_flush(&r, 2, 1, 2,
                                            10.0, 10.02, 0.075));
    TEST_ASSERT(server_stream_should_flush(&r, 3, 1, 2,
                                           10.0, 10.04, 0.075));
    TEST_ASSERT(server_stream_should_flush(&r, 2, 1, 4,
                                           10.0, 10.08, 0.075));
    TEST_ASSERT(!server_stream_should_flush(&r, 2, 1, 4,
                                            10.0, 11.0, 0.0));
}
...
static void ds4_server_unit_tests_run(void) {
    ...
    test_greedy_chain_request_shape_guards_protocol_features();
